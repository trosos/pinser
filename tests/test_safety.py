...
def test_path_safety_marks_backslash_unc_paths_as_manual_approval(tmp_path: Path) -> None:
    safety = PathSafety(tmp_path)

    decision = safety.check_read_path(r"\\server\share\file.txt")
...
def test_path_safety_rejects_traversal_before_filesystem_probe(tmp_path: Path) -> None:
    safety = PathSafety(tmp_path)

    decision = safety.check_write_path("nested/../../secret.txt")
